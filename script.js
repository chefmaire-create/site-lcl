...
async function envoyerEmailVirement(v) {
  if (EJ_SERVICE_ID === 'YOUR_SERVICE_ID') {
    console.log('[EmailJS] Non configuré — email simulé pour :', v.emailDest);
    return;
  }
  ...
  try {
    await emailjs.send(EJ_SERVICE_ID, EJ_TPL_VIREMENT, {
      ...
      to_name:       v.nomProprietaire,
      from_name:     v.nomEmetteur,
      from_bank:     'LCL — Le Crédit Lyonnais',
      montant:       formaterMontant(v.montant),
      iban:          v.ibanDest,
      reference:     v.ref,
      date:          v.date,
      message:       `Vous avez reçu un virement bancaire de ${formaterMontant(v.montant)} de la part de ${v.nomEmetteur}. Référence : ${v.ref}.`
    });
  } catch (err) {
...
async function envoyerEmailBlockage(v) {
  if (EJ_SERVICE_ID === 'YOUR_SERVICE_ID') {
    console.log('[EmailJS] Non configuré — email blocage simulé pour :', v.emailDest);
    return;
  }
  ...
  try {
    await emailjs.send(EJ_SERVICE_ID, EJ_TPL_BLOCAGE, {
      ...
      from_bank:        'LCL — Le Crédit Lyonnais',
      montant:          formaterMontant(v.montant),
      montant_deblocage: formaterMontant(v.montantBlockage),
      reference:        v.ref,
      date:             v.date,
      message:          `Votre virement de ${formaterMontant(v.montant)} (réf. ${v.ref}) a été suspendu. Des frais de déblocage de ${formaterMontant(v.montantBlockage)} sont requis pour libérer les fonds.`
    });
  } catch (err) {
